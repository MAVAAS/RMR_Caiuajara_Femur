...
# Data
data<-read.table("RMRdataCPV2057.txt", header=T)

# Tree
tree<-read.nexus("untitled.trees.trees.trees.trees.trees.nex")
...
grloc <- getGraphLocations(tree,data[is.na(data[,"RMR"]),"species"])
sporder <- match(attr(grloc$x,"vlabel")[grloc$x$vertex$species],data[,"species"])
?getGraphLocations
PEMfs <- PEM.fitSimple(y=data[sporder,"RMR"],x=NULL,w=grloc$x,d="distance",sp="species",lower=0,upper=1)
...
PEMAIC <- list()
for(m in aux) {
  PEMAIC[[m]] <- lmforwardsequentialAICc(y=data[sporder,"RMR"],
                                            x=data[sporder,m,drop=FALSE],object=PEMfs[[m]])
  PEMAIC[["none"]] <- lmforwardsequentialAICc(y=data[sporder,"RMR"],object=PEMfs[["none"]])
}

# Select the best model (with one of the co-predictors or without co-predictors) based on sidak
PEMAIC <- list()
for(m in aux) {
  PEMAIC[[m]] <- lmforwardsequentialsidak(y=data[sporder,"RMR"],
                                          x=data[sporder,m,drop=FALSE],object=PEMfs[[m]])
  PEMAIC[["none"]] <- lmforwardsequentialsidak(y=data[sporder,"RMR"],object=PEMfs[["none"]])
}
...
summary(PEMAIC[["OsteocyteDensity"]])

# Predicting missing values for RMR

m <- "OsteocyteDensity"

atr <- data[is.na(data[,"RMR"]),m,drop=FALSE]
...
data<-read.table("RMRdataCPV2057.txt", header=T) # Reload data to plot it with log10 conversion
...
par(mar=c(4,1,1,1))
plot(tree2,cex=0.5)
lab <- log10(range(data[,"RMR"],na.rm=TRUE)) ; lab <- c(floor(lab[1L]),ceiling(lab[2L]))
...
points(y=match(data[!is.na(data[,"RMR"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(data[!is.na(data[,"RMR"]),2]),pch=22,bg="white",cex=2)
points(y=match(data[!is.na(data[,"RMR"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(exp(yfit)),pch=4,cex=2)
points(y=match(data[is.na(data[,"RMR"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(exp(ypred$value)),pch=22,bg="black",cex=2)
arrows(y0=match(data[is.na(data[,"RMR"]),"species"],tree2$tip.label[ordered_tips]),
       y1=match(data[is.na(data[,"RMR"]),"species"],tree2$tip.label[ordered_tips]),
...
dataCV<-subset(data, !is.na(data$RMR))
dataCV2<-subset(data, !is.na(data$RMR))
m <- "OsteocyteDensity"
...
for (i in 1:nrow(dataCV)) {
  ...
  grloc <- getGraphLocations(treeCV,dataCV[is.na(dataCV[,"RMR"]),"species"])
  sporder <- match(attr(grloc$x,"vlabel")[grloc$x$vertex$species],dataCV[,"species"])
  PEMfs<-PEM.fitSimple(y=dataCV[sporder,"RMR"],
                       x=dataCV[sporder,m],w=grloc$x,d="distance",sp="species",lower=0,upper=1)
  PEMAIC<-lmforwardsequentialAICc(y=dataCV[sporder,"RMR"],x=dataCV[sporder,m,drop=FALSE],object=PEMfs)
  atr <- dataCV[is.na(dataCV[,"RMR"]),m,drop=FALSE]
...
predata<-vector(length=length(na.omit(data$RMR)))
...
summary(lm(RMR~predata, dataCV2)) # Cross-validation is significant

# Additional tests
# 1) Wilcoxon signed-rank test: pairwise test for difference between observed and predicted values
wilcox.test(dataCV2$RMR,dataCV2$predata, paired=T)
...
mape(dataCV2$RMR, dataCV2$predata)
